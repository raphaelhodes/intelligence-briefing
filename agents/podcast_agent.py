...
Checks RSS feeds for new episodes, scrapes transcripts (Podscripts, show websites,
YouTube captions), runs deep analysis per episode through Claude, and delivers
to Notion + Gmail.
"""
...
LOOKBACK_HOURS = 36  # Check for episodes published in last 36 hours
...
You must extract:
1. CORE THESIS — What is the central argument or claim of this episode?
2. KEY PERSPECTIVES — What are the 3-5 most important points, arguments, or insights shared?
3. NOTABLE MOMENTS — Anything surprising, contrarian, or particularly quotable?
4. CRITICAL ANALYSIS — Where do you agree/disagree with the hosts? What are they missing?
5. IMPLICATIONS FOR RAPH:
   - Google lens: what does this mean for Raph's retail media work and client conversations?
   - Spyr lens: what does this mean for building Spyr as a DTC brand?
   - Growth lens: what does this mean for Raph's thinking, positioning, or development?
6. BOTTOM LINE — One sentence. What should Raph actually do or think differently after listening to this?
...
def get_new_episodes(source: dict, lookback_hours: int = LOOKBACK_HOURS) -> list[dict]:
    ...
    if not rss_url:
        log.info(f"No RSS for {source['name']} — will search web")
    ...
    try:
        ...
        for entry in feed.entries[:10]:  # Check most recent 10
            ...
            if pub_date and pub_date > cutoff:
                ...
                log.info(f"New episode found: {source['name']} — {entry.get('title', '')}")
...
def search_web_for_episodes(source: dict, lookback_hours: int) -> list[dict]:
...
def get_transcript(episode: dict) -> str:
    """Try multiple sources to get a transcript. Returns best available text."""
    transcript = ""
...
def try_podscripts(episode: dict) -> str:
    ...
    try:
        # Build a search URL
        podcast_slug = episode["podcast"].lower().replace(" ", "-").replace("&", "and")
        ep_title_slug = episode["title"].lower().replace(" ", "-")[:60]
        ...
        for link in links:
            ...
            if episode["podcast"].lower().split("/")[0].strip().lower() in title_text or \
...
def try_episode_website(episode: dict) -> str:
    """Try to scrape the episode's own webpage for a transcript or show notes."""
    ...
    try:
        ...
        # Look for transcript or show notes section
        ...
        # Fall back to main article text
...
def try_youtube_transcript(episode: dict) -> str:
    """Try to get YouTube auto-captions via web scraping (no API key needed)."""
    # Search for the episode on YouTube to find the video ID
    try:
        ...
        # Try to get the transcript page
...
{'Analyse based on the full transcript above.' if has_transcript else 'Analyse based on the show notes and description. Note that this is based on limited information.'}
...
def build_podcast_notion_blocks(analyses: list[dict]) -> list:
...
def build_podcast_email(analyses: list[dict], date_str: str) -> str:
